...
(function ( $ ) {
    ...
    $.fn.pcsFormatNumber = function( options ) {
        ...
        function formatNumber(number, currency) {
            ...
            if(currency == null)
            {
                return number.toFixed(settings.to_fixed).replace(/(\d)(?=(\d{3})+\.)/g, '$1'+settings.number_separator).replace(settings.decimal_separator, settings.number_separator).replace(settings.number_separator, settings.decimal_separator);
            }
